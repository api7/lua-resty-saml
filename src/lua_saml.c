...
static int doc_session_expires(lua_State* L) {
  lua_settop(L, 1);
  xmlDoc* doc = doc_check(L, 1);
  lua_pop(L, 1);

  xmlChar* session_expires = saml_doc_session_expires(doc);
  if (session_expires == NULL) {
    lua_pushnil(L);
  } else {
    lua_pushstring(L, (char*)session_expires);
    xmlFree(session_expires);
  }
  return 1;
}
...
@treturn bool success
*/
static int key_add_ca_memory(lua_State* L) {
  lua_settop(L, 2);
  ...
  if (xmlSecCryptoAppKeysMngrCertLoadMemory(mngr, cert, cert_len, xmlSecKeyDataFormatPem, xmlSecKeyDataTypeTrusted) < 0) {
    lua_pushboolean(L, 0);
  } else {
    lua_pushboolean(L, 1);
  }
...
static const struct luaL_Reg saml_funcs[] = {
  ...
  {"key_add_ca_memory", key_add_ca_memory},
